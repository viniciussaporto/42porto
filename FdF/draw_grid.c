...
/*   Updated: 2023/05/31 16:15:49 by vsa-port         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	edge_case(t_edge_case_args *args)
{
	...
	{
		args->vars->rgb_p = args->vars->rgb_p \
			+ args->range / (args->vars->size_grid);
		my_mlx_pixel_put(args->vars, args->x, y, \
			percent_to_color(args->vars->rgb_p, args->vars->flag));
...
void	initializer(t_vars *vars, t_point a, t_point b, float x)
{
	...
	edge_case_args.a = a;
	edge_case_args.b = b;
	edge_case_args.range = a.z / vars->new_max;
	if (b.z - a.z > 0)
		edge_case_args.range_z = (b.z / vars->new_max - edge_case_args.range);
	else
		edge_case_args.range_z = -(a.z / vars->new_max - b.z / vars->new_max);
	edge_case_args.x = x;
